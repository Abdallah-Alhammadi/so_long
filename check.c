...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/26 13:50:04 by abalhamm          #+#    #+#             */
/*   Updated: 2023/07/26 17:42:20 by abalhamm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_map_helper(t_solong *map_line, int map_len, t_var *v)
{
	while (map_line->map[v->i] != NULL)
	{
		...
		else if (map_line->map[v->i][0] != '1' || map_line->map[v->i]
				[ft_strlen(map_line->map[v->i]) - 1] != '1')
			...
			while (map_line->map[v->i][v->j])
			{
				if (map_line->map[v->i][v->j] != '1')
					return (ft_putendl_fd
						("Error: not surrounded by walls", 2), 0);
				v->j++;
			}
		}
		v->i++;
